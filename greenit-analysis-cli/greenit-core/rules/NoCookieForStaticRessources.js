rulesManager.registerRule(
  {
    ...
    check: function (measures) {
          ...
          if (isStaticRessource(entry) && cookiesLength > 0) {
            ...
            this.detailComment +=
              chrome.i18n.getMessage(
                'rule_NoCookieForStaticRessources_DetailComment',
                entry.request.url
              ) + '<br> ';
          }
        });
      if (nbRessourcesStaticWithCookie > 0) {
        ...
        this.comment = chrome.i18n.getMessage('rule_NoCookieForStaticRessources_Comment', [
          String(nbRessourcesStaticWithCookie),
          String(Math.round(totalCookiesSize / 100) / 10)
        ]);
      }
    }
  },
