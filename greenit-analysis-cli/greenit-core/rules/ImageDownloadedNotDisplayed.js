rulesManager.registerRule(
  {
    ...
    check: function (measures) {
      measures.imagesResizedInBrowser.forEach((entry) => {
        if (!this.imgAnalysed.has(entry.src) && this.isRevelant(entry)) {
          // Do not count two times the same picture
          this.detailComment +=
            chrome.i18n.getMessage('rule_ImageDownloadedNotDisplayed_DetailComment', [
              entry.src,
      ...
      this.comment = chrome.i18n.getMessage(
        'rule_ImageDownloadedNotDisplayed_Comment',
        String(this.imageDownloadedNotDisplayedNumber)
      );
    }
  },
