          ...
          if (entry.response) {
            ...
            if (imageType !== '') {
              ...
              myImage.onload = function () {
                const minGains = getMinOptimisationGainsForImage(
                ...
                if (minGains > 500) {
                  ...
                  this.rule.detailComment +=
                    chrome.i18n.getMessage('rule_OptimizeBitmapImages_DetailComment', [
                      this.src + ' , ' + Math.round(this.size / 1000),
                      this.width + 'x' + this.height,
                      String(Math.round(minGains / 1000))
                    ]) + '<br>';
                }
                if (nbImagesToOptimize > 0) {
                  if (totalMinGains < 50000) this.rule.complianceLevel = 'B';
                  else this.rule.complianceLevel = 'C';
                  this.rule.comment = chrome.i18n.getMessage('rule_OptimizeBitmapImages_Comment', [
                    String(nbImagesToOptimize),
                    String(Math.round(totalMinGains / 1000))
                  ]);
