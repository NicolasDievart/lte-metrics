            ...
            if (isResourceCompressed(entry)) {
              ...
              this.detailComment +=
                chrome.i18n.getMessage(
                  'rule_CompressHttp_DetailComment',
                  `${entry.request.url} ${Math.round(entry.response.content.size / 100) / 10}`
                ) + '<br>';
          }
        });
      if (compressibleResourcesSize > 0) {
        const compressRatio =
          (compressibleResourcesCompressedSize / compressibleResourcesSize) * 100;
        if (compressRatio < 95) {
